...
$.extend($.fn, {
    initJerichoTab: function(setting) {
            ...
            */
            ...
            $.fn.jerichoTab = {
                ...
                addTab: function(tabsetting) {
                    ...
                    */
                    var menuurl=$('.expand').text();
                    if(menuurl==''){
                    	menuurl+=ps.title;
                    }else{
                    	menuurl=menuurl+"/"+ps.title;
                    } 
                    var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                                                    '<div class="tab_left"  style="width:' + (opts.tabWidth) + 'px"  >' +
                                                        ...
                                                        '<div class="tab_text" title="' + menuurl + '"  style="width:' + (opts.tabWidth - 50) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
        ...
        $.fn.jerichoTab.resize = function() {
            $.fn.jerichoTab.tabPageWidth = $(".tab_pages", $.fn.jerichoTab.master).width() - (($(".jericho_slider").length > 0) ? -10 : 0);
            $(".tabs", $.fn.jerichoTab.master).width($.fn.jerichoTab.tabPageWidth + 20).applySlider().adaptSlider();
            $('#jerichotab_contentholder').height($(opts.renderTo).height()-115);
        };
        $(window).resize(function() {
            $.fn.jerichoTab.resize();
            $('#jerichotab_contentholder').find("iframe").each(function(){
            	$(this).css("height", $('#jerichotab_contentholder').height());
            });
        });
    ...
    attachSliderEvent: function() {
        return this.each(function() {
            ...
            $(me).hover(function() {
                ...
                var maxLeft = (tabHash.length) * ($.fn.jerichoTab.tabWidth + 15) - $.fn.jerichoTab.tabPageWidth;
                switch ($(me).attr('pos')) {
                    case 'left':
                        //slide to the left side
                        if (offLeft + $.fn.jerichoTab.tabWidth < 0) {
                            $.fn.jerichoTab.tabpage.animate({left: offLeft + $.fn.jerichoTab.tabWidth},50);
                        }
                        else
    ...
    applySlider: function() {
        return this.each(function() {
            ...
            if (tabHash.length > 0 && offWidth > 0) {
                ...
                $.fn.jerichoTab.tabpage.parent().css({ width: $.fn.jerichoTab.tabPageWidth - 30 });
                //auto grow the tabpage(ul) width and reset 'left'
                $.fn.jerichoTab.tabpage.css({ width: offWidth + $.fn.jerichoTab.tabPageWidth + tabHash.length * 20 }).animate({ left: -offWidth - (tabHash.length-1) * 20 }, function() {
                ...
                $.fn.jerichoTab.tabpage.css({ width: -offWidth + $.fn.jerichoTab.tabPageWidth + tabHash.length * 20  }).animate({ left: 0 });
            }
        });
    },
        ...
        return this.each(function() {
            if ($('.jericho_sliders').length > 0) {
                ...
                if ((space_l < 0 && space_l > -$.fn.jerichoTab.tabWidth) && (space_r > 0 && space_r < $.fn.jerichoTab.tabWidth)) {
                    //left
                    console.log('left');
                    $.fn.jerichoTab.tabpage.animate({ left: -($.fn.jerichoTab.tabWidth + 10) * index }, function() {
                ...
                if ((space_l < tabWidth && space_l > tabWidth - $.fn.jerichoTab.tabWidth) && (space_r > tabWidth && space_r < tabWidth + $.fn.jerichoTab.tabWidth)) {
                    //right
                    console.log('right');
                    $.fn.jerichoTab.tabpage.animate({ left: -($.fn.jerichoTab.tabWidth + 10) * (index + 1) + tabWidth }, function() {
